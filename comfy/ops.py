...
from mindspore_patch.scaled_dot_product_attn import scaled_dot_product_attention
...
#         from mint.nn.attention import SDPBackend, sdpa_kernel
#         import inspect
...
#                     return mint.functional.scaled_dot_product_attention(q, k, v, *args, **kwargs)
#         else:
...
def cast_bias_weight(s, input=None, dtype=None, device=None, bias_dtype=None, offloadable=False):
    ...
    weight = comfy.model_management.cast_to(s.weight, None, None, non_blocking=False, copy=weight_has_function, stream=offload_stream)

    bias = None
    if s.bias is not None:
        bias = comfy.model_management.cast_to(s.bias, bias_dtype, None, non_blocking=False, copy=bias_has_function, stream=offload_stream)
...
class disable_weight_init:
    class Linear(mint.nn.Linear, CastWeightBiasOp):
        ...
        def forward_comfy_cast_weights(self, input):
            ...
            x = mint.functional.linear(input, weight, bias)
    ...
    class Conv3d(mint.nn.Conv3d, CastWeightBiasOp):
        ...
        def _conv_forward(self, input, weight, bias, *args, **kwargs):
            if NVIDIA_MEMORY_CONV_BUG_WORKAROUND and weight.dtype in (mindspore.float16, mindspore.bfloat16):
                raise NotImplementedError
            else:
    ...
    class GroupNorm(mint.nn.GroupNorm, CastWeightBiasOp):
        ...
        def forward_comfy_cast_weights(self, input):
            weight, bias = cast_bias_weight(self, input, offloadable=False)
            x = mint.functional.group_norm(input, self.num_groups, weight, bias, self.eps)
            uncast_bias_weight(self, weight, bias, None)
            return x
    ...
    class RMSNorm(comfy.rmsnorm.RMSNorm, CastWeightBiasOp):
        ...
        def forward_comfy_cast_weights(self, input):
            ...
            # x = mint.functional.rms_norm(input, self.normalized_shape, weight, self.eps)
            uncast_bias_weight(self, weight, bias, None)
            return x
    ...
    class ConvTranspose2d(mint.nn.ConvTranspose2d, CastWeightBiasOp):
        ...
        def forward_comfy_cast_weights(self, input, output_size=None):
            ...
            weight, bias = cast_bias_weight(self, input, offloadable=False)
            x = mint.functional.conv_transpose2d(
...
#                     x = mint.functional.linear(input, weight * self.scale_weight.to(device=weight.device, dtype=weight.dtype), bias)
#                 else:
#                     x = mint.functional.linear(input * self.scale_weight.to(device=weight.device, dtype=weight.dtype), weight, bias)
#                 uncast_bias_weight(self, weight, bias, offload_stream)
#                 return x
